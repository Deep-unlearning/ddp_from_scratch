...
from ddp import get_rank, get_world_size
...
class ShardedOptimizer:
    ...
    def step(self, closure=None):
        ...
        for i, param in enumerate(self.params):
            ...
            # shard per rank.
            in_tensor = torch.cat([flattened_grad for _ in range(get_world_size())], dim=0)
            ...
            if i in self.local_param_indices:
                param.grad.data = (output_tensor / get_world_size()).view_as(grad.data)
            else:
                param.grad = None
        ...
        params_per_rank = len(self.params) // get_world_size()
        remainder = len(self.params) % get_world_size()
...
def test_zero2():
    ...
    if rank == 0:
        ...
        print(f"Peak memory with ZeRO-2 sharded Adam: {peak_memory_z1:.2f} MB")
        print(
